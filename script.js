...
class DiscordTracker {
  ...
  async performSearch() {
    ...
    try {
      ...
      // First try to get user from external API
      let userData = null;
      let source = 'external';
      
      try {
        const externalResponse = await fetch(`https://fishonxd.su/api/checks/user/${query}`);
        if (externalResponse.ok) {
          const externalData = await externalResponse.json();
          if (externalData.success && externalData.data && externalData.data.user) {
            userData = externalData.data;
            console.log('✅ User found via external API:', userData);
          }
        }
      } catch (externalError) {
        console.log('⚠️ External API not available, trying internal API');
      }
      
      // If external API failed, try internal API
      if (!userData) {
        try {
                ...
                if (sessionData.data.isAuthenticated && sessionData.data.user) {
                  ...
                  if (retryResponse.ok) {
                    ...
                    if (retryData.success) {
                      ...
                      source = 'internal';
                      console.log('✅ User found via internal API retry:', userData);
                    }
                  }
                }
              }
          ...
          if (response.ok) {
            ...
            if (data.success) {
              userData = data.data;
              source = 'internal';
              console.log('✅ User found via internal API:', userData);
            }
          }
        } catch (internalError) {
          console.log('❌ Internal API error:', internalError);
        }
      }
      
      if (userData) {
        // Display user profile with enhanced data
        this.displayEnhancedUserProfile(userData, source);
        this.showNotification(`Профиль успешно загружен! (${source === 'external' ? 'Внешний API' : 'Внутренний API'})`, 'success');
      } else {
        // Show "Unknown User" message for users not in database
        this.showUnknownUserMessage(query);
      }
    } catch (error) {
  ...
  // New enhanced profile display function for external API data
  displayEnhancedUserProfile(userData, source) {
    this.currentUser = userData.user || userData;
    
    // Show enhanced profile section
    document.getElementById('enhanced-profile-section').style.display = 'block';
    document.getElementById('profile-section').style.display = 'none';
    document.getElementById('dashboard').style.display = 'none';
    document.getElementById('user-dashboard').style.display = 'none';

    // Update enhanced profile with all API data
    this.updateEnhancedProfileInfo(userData);
    this.updateEnhancedQuickStats(userData);
    this.updateEnhancedRiskAssessment(userData);
    this.updateEnhancedTabs(userData);
    this.updateTrackButton(this.currentUser);
    
    // Show data source indicator
    this.showDataSourceIndicator(source);
    
    this.triggerProfileAnimations();
    
    // Load reputation and views if available
    if (this.currentUser.id) {
      this.fetchAndDisplayReputation(this.currentUser.id);
      this.fetchAndDisplayViews(this.currentUser.id);
      this.autoAddView(this.currentUser.id);
    }
  }
  ...
  // Enhanced profile update functions for external API data
  updateEnhancedProfileInfo(userData) {
    const user = userData.user || userData;
    
    // Update avatar
    const userAvatarProfile = document.getElementById('enhanced-user-avatar');
    if (userAvatarProfile && user.avatar) {
      userAvatarProfile.src = user.displayAvatarURL || `https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}.webp`;
    }
    
    // Update username and display name
    const username = document.getElementById('enhanced-username');
    if (username) {
      username.textContent = user.displayName || user.username || 'Неизвестно';
    }
    
    const userTag = document.getElementById('enhanced-user-tag');
    if (userTag) {
      userTag.textContent = `@${user.username}`;
    }
    
    const userDiscriminator = document.getElementById('enhanced-user-discriminator');
    if (userDiscriminator) {
      userDiscriminator.textContent = `#${user.discriminator}`;
    }
    
    // Update bio
    const userBio = document.getElementById('enhanced-user-bio');
    if (userBio) {
      userBio.textContent = user.bio || 'Описание не указано';
    }
    
    // Update badges
    const badgesContainer = document.getElementById('enhanced-user-badges');
    if (badgesContainer) {
      badgesContainer.innerHTML = '';
      
      if (user.clan && user.clan.tag) {
        const clanBadge = document.createElement('span');
        clanBadge.className = 'badge clan-badge';
        clanBadge.innerHTML = `
          <img src="${user.clanBadgeURL || ''}" alt="Clan Badge" class="clan-badge-icon">
          ${user.clan.tag}
        `;
        badgesContainer.appendChild(clanBadge);
      }
      ...
      if (user.avatarDecoration) {
        ...
        decorationBadge.className = 'badge decoration-badge';
        decorationBadge.innerHTML = `
          <img src="${user.avatarDecorationURL || ''}" alt="Decoration" class="decoration-icon">
          Декорация
        `;
        badgesContainer.appendChild(decorationBadge);
      }
    }
  }

  updateEnhancedQuickStats(userData) {
    const user = userData.user || userData;
    
    // Update message count
    const messageCountElement = document.getElementById('enhanced-message-count');
    if (messageCountElement) {
      messageCountElement.textContent = userData.messageCount || 0;
    }
    
    // Update account age
    const accountAgeElement = document.getElementById('enhanced-account-age');
    if (accountAgeElement && user.createdAt) {
    ...
    // Update voice time
    const voiceTimeElement = document.getElementById('enhanced-voice-time');
    if (voiceTimeElement && userData.lastVoiceChannel) {
      const timeInSeconds = userData.lastVoiceChannel.time || 0;
    ...
    // Update reputation
    const reputationElement = document.getElementById('enhanced-reputation');
    if (reputationElement && userData.reputation) {
      const positive = userData.reputation.positive || 0;
      const negative = userData.reputation.negative || 0;
      reputationElement.textContent = `${positive} / ${negative}`;
    }
  }

  updateEnhancedRiskAssessment(userData) {
    const user = userData.user || userData;
    
    const riskFill = document.getElementById('enhanced-risk-fill');
    const riskScore = document.getElementById('enhanced-risk-score');
    ...
    if (riskFill && riskScore) {
      ...
      // Calculate risk based on message count
      if (userData.messageCount && userData.messageCount < 100) {
        riskScoreValue += 20;
      }
      
      // Calculate risk based on voice activity
      if (userData.lastVoiceChannel && userData.lastVoiceChannel.time === 0) {
        riskScoreValue += 15;
      }
      
      // Calculate risk based on mutual servers
      if (userData.mutualGuilds && userData.mutualGuilds.servers) {
        const suspiciousServers = userData.mutualGuilds.servers.filter(server => 
          server.guild.memberCount < 1000 || 
          server.guild.description?.toLowerCase().includes('scam') ||
          server.guild.description?.toLowerCase().includes('hack')
        ).length;
        riskScoreValue += suspiciousServers * 10;
      }
  ...
  updateEnhancedTabs(userData) {
    const user = userData.user || userData;
    
    // Update mutual servers
    const mutualServersList = document.getElementById('enhanced-mutual-servers');
    if (mutualServersList && userData.mutualGuilds && userData.mutualGuilds.servers) {
      mutualServersList.innerHTML = userData.mutualGuilds.servers.map(server => `
        <div class="enhanced-server-item">
          <div class="server-header">
            <img src="${server.guild.iconURL || ''}" alt="${server.guild.name}" class="server-icon" onerror="this.style.display='none'">
            <div class="server-info">
              <h4>${this.escapeHtml(server.guild.name)}</h4>
              <p>${this.escapeHtml(server.guild.description || 'Описание отсутствует')}</p>
            </div>
          </div>
          <div class="server-stats">
            <span class="stat">
              <i class="fas fa-users"></i>
              ${server.guild.memberCount.toLocaleString()}
            </span>
            <span class="stat">
              <i class="fas fa-microphone"></i>
              ${server.guild.memberInVoice.toLocaleString()}
            </span>
            <span class="stat">
              <i class="fas fa-star"></i>
              Tier ${server.guild.boosts.tier} (${server.guild.boosts.count})
            </span>
          </div>
          <div class="server-actions">
            ${server.guild.invite ? `
              <a href="${server.guild.invite}" target="_blank" class="btn btn-primary btn-sm">
                <i class="fas fa-sign-in-alt"></i>
                Присоединиться
              </a>
            ` : ''}
            <span class="server-created">Создан: ${server.guild.createdAt}</span>
          </div>
        </div>
      `).join('');
    }
    ...
    const usernameHistoryList = document.getElementById('enhanced-username-history');
    if (usernameHistoryList && userData.lastUsernames) {
      usernameHistoryList.innerHTML = userData.lastUsernames.map(nameChange => `
        <div class="username-change-item">
          <div class="change-info">
            <span class="old-name">${this.escapeHtml(nameChange.username.old)}</span>
            <i class="fas fa-arrow-right"></i>
            <span class="new-name">${this.escapeHtml(nameChange.username.new)}</span>
          </div>
          <div class="change-meta">
            <span class="guild-name">${this.escapeHtml(nameChange.guildId)}</span>
            <span class="change-date">${this.formatDate(nameChange.updatedAt)}</span>
          </div>
        </div>
      `).join('');
    }
    
    // Update last voice channel
    const lastVoiceElement = document.getElementById('enhanced-last-voice');
    if (lastVoiceElement && userData.lastVoiceChannel) {
      const voiceData = userData.lastVoiceChannel;
      lastVoiceElement.innerHTML = `
        <div class="voice-channel-info">
          <div class="channel-header">
            <i class="fas fa-microphone"></i>
            <span class="channel-name">${this.escapeHtml(voiceData.channel.name)}</span>
          </div>
          <div class="channel-server">
            <i class="fas fa-server"></i>
            <span>${this.escapeHtml(voiceData.guild.name)}</span>
          </div>
          <div class="channel-time">
            <i class="fas fa-clock"></i>
            <span>${this.formatTime(voiceData.time)}</span>
          </div>
          <div class="channel-dates">
            <span>Вход: ${this.formatDate(voiceData.joinTime)}</span>
            <span>Выход: ${this.formatDate(voiceData.leaveTime)}</span>
          </div>
        </div>
      `;
    }
  ...
  showDataSourceIndicator(source) {
    ...
    if (indicator) {
      indicator.textContent = source === 'external' ? 'Внешний API' : 'Внутренний API';
      indicator.className = `data-source-indicator ${source}`;
    }
    
    // Bind tab switching functionality
    this.bindEnhancedTabEvents();
  }

  bindEnhancedTabEvents() {
    const tabButtons = document.querySelectorAll('.enhanced-profile-tabs .tab-btn');
    const tabPanels = document.querySelectorAll('.enhanced-profile-tabs .tab-panel');
    ...
    // Bind enhanced track button
    const enhancedTrackBtn = document.getElementById('enhanced-track-btn');
    if (enhancedTrackBtn) {
      enhancedTrackBtn.addEventListener('click', () => {
        if (this.currentUser && this.currentUser.id) {
          this.addToTracking(this.currentUser.id);
        }
      });
    }
  }
  ...
  // Show "Unknown User" message for users not in database
  showUnknownUserMessage(query) {
    // Hide all sections
    this.hideAllSections();
    
    // Show unknown user message
    const unknownUserSection = document.getElementById('unknown-user-section');
    if (unknownUserSection) {
      unknownUserSection.style.display = 'block';
      
      // Update the message with the searched Discord ID
      const unknownUserId = document.getElementById('unknown-user-id');
      if (unknownUserId) {
        unknownUserId.textContent = query;
      }
      
      // Show notification
      this.showNotification('Пользователь не найден в базе данных. Возможно, он не зарегистрирован или не закеширован.', 'warning');
    } else {
      // Fallback: show notification only
      this.showNotification(`Пользователь с ID ${query} не найден в базе данных`, 'warning');
    }
  }
