...
class DiscordTracker {
  ...
  async loadUserProfileData(discordId) {
    try {
      ...
      let response = await fetch(`${this.apiBaseUrl}/user/${discordId}`, {
        credentials: 'include'
      });
      ...
      if (response.ok) {
        ...
        console.log('[DEBUG] User profile data loaded from /user/:', responseData);
        
        // Extract user data from the response structure
        if (responseData.data && responseData.data.user) {
          userData = responseData.data.user;
          console.log('[DEBUG] Extracted user data from responseData.data.user:', userData);
        } else if (responseData.user) {
          userData = responseData.user;
          console.log('[DEBUG] Extracted user data from responseData.user:', userData);
        } else {
          userData = responseData;
          console.log('[DEBUG] Using responseData directly as userData:', userData);
        }
      } else {
        console.warn('[DEBUG] Failed to load from /user/, trying /search...');
        
        // Try the search endpoint as fallback
        response = await fetch(`${this.apiBaseUrl}/search?query=${discordId}`, {
          credentials: 'include'
        });
        ...
        if (response.ok) {
          ...
          // Extract user data from search response
          let users = null;
          if (searchData.data && searchData.data.users) {
          ...
          if (users && users.length > 0) {
            userData = users[0];
            console.log('[DEBUG] User profile data loaded from /search:', userData);
          }
        } else {
          console.warn('[DEBUG] Failed to load from /search, Status:', response.status);
      ...
      if (userData) {
        // Update tracking list with complete user data
        if (this.trackingList) {
          const trackingIndex = this.trackingList.findIndex(user => user.userId === discordId);
          if (trackingIndex !== -1) {
            // Merge the complete user data with tracking data
            this.trackingList[trackingIndex] = {
              ...this.trackingList[trackingIndex],
              username: userData.username || userData.displayName || userData.globalName,
              avatar: userData.avatar,
              discriminator: userData.discriminator,
              status: userData.status || 'offline',
              activity: userData.activity || {},
              accountAge: userData.accountAge || this.calculateAccountAge(userData.createdAt),
              displayAvatarURL: userData.displayAvatarURL,
              bio: userData.bio,
              clan: userData.clan,
              messageCount: userData.messageCount
            };
            console.log('[DEBUG] Updated tracking list with complete user data:', this.trackingList[trackingIndex]);
            
            // Re-render the tracking list to show updated data
            this.renderTrackingList(this.trackingList);
          }
        }
      } else {
        console.warn('[DEBUG] No user data found for:', discordId);
      }
    } catch (error) {
      console.error('[DEBUG] Error loading user profile data:', error);
    }
  }
