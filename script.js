...
class DiscordTracker {
  ...
  updateDiscordProfileExtras(userData) {
    // Show all user data from the provided JSON
    const profileInfo = document.getElementById('user-profile-info');
    if (!profileInfo) return;
    let html = '';
    // Основные поля
    html += `<div><b>ID:</b> ${userData.id}</div>`;
    html += `<div><b>Username:</b> ${userData.username}</div>`;
    html += `<div><b>Global Name:</b> ${userData.globalName || ''}</div>`;
    html += `<div><b>Discriminator:</b> ${userData.discriminator}</div>`;
    html += `<div><b>Display Name:</b> ${userData.displayName || ''}</div>`;
    html += `<div><b>Tag:</b> ${userData.tag || ''}</div>`;
    html += `<div><b>Bio:</b> ${userData.bio || ''}</div>`;
    html += `<div><b>Gender:</b> ${userData.gender || ''}</div>`;
    html += `<div><b>System:</b> ${userData.system ? 'Да' : 'Нет'}</div>`;
    html += `<div><b>Flags:</b> ${userData.flags}</div>`;
    html += `<div><b>Accent Color:</b> <span style='color: #${userData.accentColor?.toString(16).padStart(6, '0')};'>#${userData.accentColor?.toString(16).padStart(6, '0')}</span></div>`;
    html += `<div><b>Banner Color:</b> <span style='color: ${userData.bannerColor};'>${userData.bannerColor}</span></div>`;
    html += `<div><b>Created At:</b> ${userData.createdAt}</div>`;
    html += `<div><b>Avatar:</b> <img src='${userData.displayAvatarURL}' alt='avatar' style='width:48px;height:48px;border-radius:50%;vertical-align:middle;'> (${userData.avatar})</div>`;
    if (userData.banner) html += `<div><b>Banner:</b> ${userData.banner}</div>`;
    if (userData.avatarDecoration) {
      html += `<div><b>Avatar Decoration:</b> <img src='${userData.avatarDecorationURL}' alt='avatar decoration' style='width:48px;height:48px;vertical-align:middle;'> (asset: ${userData.avatarDecoration.asset}, sku_id: ${userData.avatarDecoration.sku_id})</div>`;
    }
    html += `<div><b>Avatar Decoration SKUId:</b> ${userData.avatarDecorationSKUId || ''}</div>`;
    // Клан
    if (userData.clan) {
      html += `<div><b>Клан:</b> ${userData.clan.tag} <img src='${userData.clanBadgeURL}' alt='clan badge' style='width:32px;height:32px;vertical-align:middle;'> (guild_id: ${userData.clan.identity_guild_id})</div>`;
    }
    // Репутация
    if (userData.reputation) {
      html += `<div><b>Репутация:</b> +${userData.reputation.positive} / -${userData.reputation.negative}</div>`;
    }
    // Сообщения
    html += `<div><b>Сообщений:</b> ${userData.messageCount}</div>`;
    // Последний голосовой канал
    if (userData.lastVoiceChannel) {
      html += `<div><b>Последний голосовой канал:</b> ${userData.lastVoiceChannel.channel.name} (сервер: ${userData.lastVoiceChannel.guild.name})</div>`;
      html += `<div><b>Время в голосе:</b> ${userData.lastVoiceChannel.time || ''}</div>`;
      html += `<div><b>Время входа:</b> ${userData.lastVoiceChannel.joinTime || ''}</div>`;
      html += `<div><b>Время выхода:</b> ${userData.lastVoiceChannel.leaveTime || ''}</div>`;
    }
    // Старые ники
    if (userData.lastUsernames && userData.lastUsernames.length > 0) {
      html += '<div><b>Старые ники:</b><ul>';
      userData.lastUsernames.forEach(entry => {
        html += `<li>${entry.username.old} → ${entry.username.new} (сервер: ${entry.guildId}, обновлено: ${entry.updatedAt})</li>`;
      });
      html += '</ul></div>';
    }
    // Мутуал гильдии
    if (userData.mutualGuilds && userData.mutualGuilds.servers) {
      html += '<div><b>Общие серверы:</b><ul>';
      userData.mutualGuilds.servers.forEach(server => {
        html += `<li>${server.guild.name} (${server.guild.id}) — ${server.guild.description || ''} <img src='${server.guild.iconURL}' alt='icon' style='width:24px;height:24px;vertical-align:middle;'>`;
        if (server.nickname) html += ` (ник: <b>${server.nickname}</b>)`;
        if (server.guild.invite) html += ` <a href='${server.guild.invite}' target='_blank'>[Инвайт]</a>`;
        html += `</li>`;
      });
      html += '</ul></div>';
    }
    // Всё остальное (отладка)
    html += `<details><summary>RAW JSON</summary><pre style='font-size:10px;white-space:pre-wrap;'>${JSON.stringify(userData, null, 2)}</pre></details>`;
