...
app.use(cors({
  origin: [
    ...
    // Добавляем поддержку всех доменов для продакшена
    /^https?:\/\/.*$/
  ],
  credentials: true
}));
    ...
    try {
      ...
      if (!trackedUser) {
        try {
          ...
          // Fetch from external API with retry logic (direct call - faster and more reliable)
          ...
          if (apiError.response?.status === 403) {
            console.log('⚠️ API access denied, creating fallback user data...');
            
            // Create fallback user data when API is inaccessible
            try {
              trackedUser = new TrackedUser({
                discordId: discordId,
                ...
                discriminator: '0000',
                avatar: null,
                bio: 'Data unavailable - External API access denied',
                nitro: false,
                clan: null,
                badges: [],
                createdAt: new Date(),
              ...
              return res.json({ 
                ...
                data: { 
                  user: trackedUser,
                  warning: 'External API access denied - showing limited user data'
                } 
              });
            } catch (fallbackError) {
            ...
            return res.status(429).json({ 
              ...
              message: 'Too many requests to the external service. Please try again later.',
              details: apiError.response?.data || 'Rate limit exceeded'
            });
            ...
            return res.status(408).json({ 
              ...
              details: 'Request timeout after 20 seconds'
            });
          }
...
try {
  const server = app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`📱 Frontend: https://ds-chekcer-1.onrender.com`);
    console.log(`🔧 Backend: https://ds-chekcer-1.onrender.com`);
