...
try {
  ...
  app.get('/auth/discord/callback',
    ...
    // Rate limiting middleware теперь после успешной авторизации
    (req, res, next) => {
      const now = Date.now();
      const lastAttempt = req.session.lastOAuthAttempt || 0;
      const timeSinceLastAttempt = now - lastAttempt;
      if (timeSinceLastAttempt < 2000) { // 2 секунды между успешными попытками
        console.warn('⚠️ OAuth rate limiting: too many attempts');
        console.log(`⏳ Time since last attempt: ${timeSinceLastAttempt}ms, minimum required: 2000ms`);
        return res.redirect('/?error=rate_limited&retry_after=2');
      }
      req.session.lastOAuthAttempt = now;
      next();
    },
    createOAuthRetryHandler(5, 3000),
    (req, res) => {
      try {
