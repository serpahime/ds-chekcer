...
// OAuth retry utility function
const createOAuthRetryHandler = (maxRetries = 3, baseDelay = 1000) => {
  return async (req, res, next) => {
    ...
    console.log(`⏳ OAuth retry ${retryCount + 1}/${maxRetries} with ${delay}ms delay`);
  ...
  passport.use(new DiscordStrategy({
    ...
    try {
      ...
      if (error.oauthError && error.oauthError.statusCode === 429) {
        console.warn('⚠️ Discord rate limiting detected, suggesting retry');
...
try {
  ...
  app.get('/auth/discord/callback', 
    ...
    createOAuthRetryHandler(3, 1000), // Add retry logic with exponential backoff
  ...
  if (error.name === 'InternalOAuthError') {
    ...
    if (error.oauthError?.statusCode === 400) {
      return res.status(400).json({
        ...
        details: 'The authorization code may have expired or is invalid.'
      });
    }
    ...
    return res.status(500).json({
      ...
      details: error.oauthError?.data || 'Unknown OAuth error'
    });
  }
